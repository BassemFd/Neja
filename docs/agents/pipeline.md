...
4. **visual-reviewer** takes the doc handoff, re-runs the palette validator, checks contrast/state-coverage/accessibility conventions, and either approves (`status: "ready"`) or rejects with structured `findings` that route back to `component-builder`.
